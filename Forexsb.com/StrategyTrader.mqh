...
TickType StrategyTrader::GetTickType(bool isNewBar, int closeAdvance)
{
    ...
    if (((market.BarTime + market.Period * 60) - market.TickServerTime) < closeAdvance)
    {
...
void StrategyTrader::CalculateTrade(TickType ticktype)
{
    ...
    if (closeStrPriceType != StrategyPriceType_CloseAndReverse &&
        (market.PositionDirection == PosDirection_Short ||
         market.PositionDirection == PosDirection_Long))
    {
        if (ticktype == TickType_Open && closeStrPriceType == StrategyPriceType_Close &&
        ...
        else if (((closeStrPriceType == StrategyPriceType_Open) &&
        ...
        else if (closeStrPriceType == StrategyPriceType_Close &&
                 openStrPriceType != StrategyPriceType_Close &&
                 ticktype == TickType_AfterClose)
    ...
    if (useLogicalGroups)
    {
        for (int i = 0; i < closingLogicGroups.Count(); i++)
        {
            ...
            for (int slot = strategy.CloseSlotNumber() + 1; slot < strategy.Slots(); slot++)
            {
                ...
                if (strategy.Slot[slot].LogicalGroup == group ||
                    strategy.Slot[slot].LogicalGroup == "all")
                {
                    for (int c = 0;
                         c < strategy.Slot[slot].IndicatorPointer.Components();
                         c++)
                        if (strategy.Slot[slot].IndicatorPointer.Component[c].GetLastValue() > 0)
                            slotDirection = GetClosingDirection(slotDirection,
                                                                strategy.Slot[slot].IndicatorPointer.Component[c].DataType);
