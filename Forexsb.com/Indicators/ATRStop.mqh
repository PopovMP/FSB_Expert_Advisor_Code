...
//|                                                                  |
//+------------------------------------------------------------------+
void ATRStop::ATRStop(SlotTypes slotType)
  {
   SlotType          = slotType;
...
void ATRStop::Calculate(DataSet &dataSet)
  {
   ...
   double multipl=NumParam[1].Value;
   ...
   for(int bar=firstBar; bar<Data.Bars-previous; bar++)
     {
      atrStop[bar+previous]=MathMax(atr[bar]*multipl,minStop);
     }
   ...
   Component[1].CompName = "ATR Stop previous pos";
