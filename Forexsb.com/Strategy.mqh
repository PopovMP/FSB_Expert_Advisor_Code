...
public:
   ...
   void              SetPeriod(int period)      {m_period   = EnumTimeFramesToPeriod(period);}
   void              SetIsTester(bool isTester) {m_isTester = isTester;}
   string            GetSymbol()                {return m_symbol;}
   DataPeriod        GetPeriod()                {return m_period;}
   bool              IsTester()                 {return m_isTester;}
   int               OpenSlots()                {return m_openSlots;};
   int               CloseSlots()               {return m_closeSlots;};
   int               Slots()                    {return m_openSlots + m_closeSlots + 2;}
   int               CloseSlotNumber()          {return m_openSlots + 1;}
   SlotTypes         GetSlotType(int slotNumber);
   void              GetRequiredCharts(string &charts[]);
   bool              IsUsingLogicalGroups();
...
SlotTypes Strategy::GetSlotType(int slotNumber)
  {
   if(slotNumber == 0)                      return (SlotTypes_Open);
   else if(slotNumber <  CloseSlotNumber()) return (SlotTypes_OpenFilter);
   else if(slotNumber == CloseSlotNumber()) return (SlotTypes_Close);
   else                                     return (SlotTypes_CloseFilter);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
bool Strategy::IsLongerTimeFrame(int slotNumber)
  {
   return !(Slot[slotNumber].IndicatorSymbol=="" && Slot[slotNumber].IndicatorPeriod==DataPeriod_M1);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
void Strategy::GetRequiredCharts(string &charts[])
  {
   ...
   for(int i=0; i<Slots(); i++)
     {
      if(!Slot[i].IndicatorPointer.IsAllowLTF) continue;
      if(!IsLongerTimeFrame(i)) continue;
   ...
   for(int i=0; i<Slots(); i++)
     {
      ...
      for(int j=0; j<ArraySize(dataSet); j++)
        {
         if(dataSet[j].Chart!=chart) continue;
   ...
   if(slotType==SlotTypes_CloseFilter)
     {
      ...
      for(int i = OpenSlots() + 2; i < Slots(); i++)
         if(Slot[i].LogicalGroup==group) count++;
      if(count>1)
         return (true);
     }
   return (false);
      ...
      for(int i=0; i<Slot[slot].IndicatorPointer.Components(); i++)
        {
         ...
         string sFr0=dl0<10?"%10.5f":dl0<100?"%10.5f":dl0<1000?"%10.3f":dl0<10000?"%10.3f":dl0<100000?"%10.2f":"%10.1f";
         string sFr1=dl1<10?"%10.5f":dl1<100?"%10.5f":dl1<1000?"%10.3f":dl1<10000?"%10.3f":dl1<100000?"%10.2f":"%10.1f";
         string format=sFr1+"    "+sFr0;
         if(component.ShowInDynInfo)
           {
            if(type==IndComponentType_AllowOpenLong || type==IndComponentType_AllowOpenShort ||
               type==IndComponentType_ForceClose    || type==IndComponentType_ForceCloseLong ||
      ...
      for(int i=0; i<Slot[slot].IndicatorPointer.Components(); i++)
        {
         ...
         string sFr=dl<10?"%10.6f":dl<100?"%10.5f":dl<1000?"%10.4":dl<10000?"%10.3f":dl<100000?"%10.2f":"%10.1f";
         string format="\n%-40s "+sFr+"    "+sFr+"    "+sFr;
         if(component.ShowInDynInfo)
           {
            if(type==IndComponentType_AllowOpenLong || type==IndComponentType_AllowOpenShort || 
               type==IndComponentType_ForceClose    || type==IndComponentType_ForceCloseLong ||
               type==IndComponentType_ForceCloseShort)
               info+=StringFormat("\n%-42s %-10s    %-10s    %-10s",name,(value2<1?"No":"Yes"),(value1<1?"No":"Yes"),(value0<1?"No":"Yes"));
            else
